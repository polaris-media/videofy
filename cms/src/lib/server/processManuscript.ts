...
import { manuscriptSchema, processedManuscriptSchema } from "@videofy/types";
...
export async function processProjectManuscript(params: {
  manuscript: ManuscriptType;
  projectId: string;
  audioMode?: "none" | "elevenlabs";
}): Promise<ProcessedManuscript> {
  const { manuscript, projectId, audioMode = "elevenlabs" } = params;
  const validatedManuscript = manuscriptSchema.parse(manuscript);
  const localizedManuscript = await localizeRemoteVideos(validatedManuscript, projectId);
